...
import {batchDeleteOAuth2User, deleteOAuth2User, getOAuth2UserById, getOAuth2UserPage} from "@/services/oauth/oauth2-user";
import type {PageParams} from "@/services/common/typings";
import {convertPageParams} from "@/utils/common";
import CreateForm from "@/pages/OAuth/OAuthUserList/components/CreateForm";
import UpdateForm from "@/pages/OAuth/OAuthUserList/components/UpdateForm";
import ViewForm from "@/pages/OAuth/OAuthUserList/components/ViewForm";
import {OAuth2UserVO} from "@/services/oauth/type/oauth2-user";

/**
 * 删除用户
 * @param selectedRows
 */
const handleRemove = async (selectedRows: OAuth2UserVO[]) => {
  const hide = message.loading('正在删除');
  if (!selectedRows) return true;
  try {
    await batchDeleteOAuth2User(selectedRows.map((row) => row.id));
    hide();
...
const OAuthUserList: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<OAuth2UserVO>();
  const [selectedRowsState, setSelectedRows] = useState<OAuth2UserVO[]>([]);

  const columns: ProColumns<OAuth2UserVO>[] = [
    {
      dataIndex: 'index',
    ...
    },
    {
      ...
      render: (_, entity) => [
        ...
        <Popconfirm
          ...
          onConfirm={async () => {
            if (entity && entity.id) {
              await deleteOAuth2User(entity.id);
              message.success('删除成功！');
  ...
  return (
    <PageContainer>
      <ProTable<OAuth2UserVO, PageParams>
        headerTitle="用户列表"
        actionRef={actionRef}
        rowKey="id"
        search={{
        ...
        request={async (params, sort) => {
          const { data } = await getOAuth2UserPage(convertPageParams(params, sort));
          return {
            // success 请返回 true，
